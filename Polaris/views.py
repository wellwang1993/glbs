...
class Zonetypeinfo(MyModelViewSet):
    ...
    def push_zone(self,request):
        try:
            ...
            if zone_name == None or len(zone_name) == 0:
...
class DnsIpinfo(MyModelViewSet):
    ...
    def get_serializer_class(self):
        if self.action in ['list','retrieve','universal_matching_dnsip']:
    ...
    def universal_matching_dnsip(self,request):
        try:
            ...
            return Response({'code':0,'msg':str(err)},status=status.HTTP_400_BAD_REQUEST)
#对后端zone的支持
class DnsBackendzoneinfo(MyModelViewSet):
    queryset = tb_fact_backend_dnszone_info.objects.all()
    def get_serializer_class(self):
        if self.action in ['list','retrieve']:
            return DnszoneListBackendSerializer
        return DnszoneUpdateBackendSerializer
...
class Dnszoneinfo(MyModelViewSet):
    ...
    def push_zone(self,request):
        try:
            ...
            if zonename == None or len(zonename) == 0:
                return Response({'code':0,'msg':"Illegal input"},status=status.HTTP_400_BAD_REQUEST)
            with transaction.atomic():
                queryset = tb_fact_dnszone_info.objects.filter(zone_name_id__zone_name=zonename)
                tb_fact_backend_dnszone_info.objects.filter(zone_name_id__zone_name=zonename).delete()
                if queryset and len(queryset) != 0:
                    for zone in queryset:
                        serializer = DnszoneUpdateBackendSerializer(data={"zone_name":zone.zone_name_id,"record_name":zone.record_name,"record_content":zone.record_content,"record_type":zone.record_type,"internet_type":zone.internet_type,"record_ttl":zone.record_ttl,"dns_type":zone.dns_type_id})
                        #print({"zone_name":zone.zone_name_id,"record_name":zone.record_name,"record_content":zone.record_content,"record_type":zone.record_type,"internet_type":zone.internet_type,"record_ttl":zone.record_ttl,"dns_type":zone.dns_type_id})
                        if not serializer.is_valid():
                            raise serializers.ValidationError
                        serializer.save()
                    return Response({'code':1,'msg':"ok"},status=status.HTTP_200_OK)
                else:
                    return Response({'code':1,'msg':"the zone name is not existx"},status=status.HTTP_400_BAD_REQUEST)
        except Exception as err:
            return Response({'code':0,'msg':str(err)},status=status.HTTP_400_BAD_REQUEST)
    #支持通过zonename查找 item
    @action(detail=False,methods=['get'])
    def universal_matching_dnszone(self,request):
        try:
            ...
            queryset = tb_fact_dnszone_info.objects.all()
            if zonename != None:
                queryset = queryset.filter(zone_name_id__zone_name__contains = zonename)
            page = self.paginate_queryset(queryset)
            if page is not None:
                serializer = self.get_serializer(page, many=True)
                return self.get_paginated_response(serializer.data)
...
class VipDeviceinfo(MyModelViewSet):
    ...
    def get_all_resource_info(self,request):
         try:
            ...
            queryset = tb_fact_device_info.objects.all()
            if country != None:
                queryset = queryset.filter(node_country__contains=country)
            if isp != None:
                queryset = queryset.filter(node_isp__contains=isp)
            if region != None:
                queryset = queryset.filter(node_region__contains=region)
            if province != None:
                queryset = queryset.filter(node_province__contains=province)
            if city != None:
                queryset = queryset.filter(node_city__contains=city)
            if nodeid != None:
    ...
    def maintain_resources(self,request):
        record_list = []
        try:
            with transaction.atomic():            
        ...
        try:
            with transaction.atomic():            
                ...
                if querydic != None and len(querydic) ==3:
                    paramsstatus = querydic.get('status')
                    noderesource = querydic.get('noderesource')
                    ipresource = querydic.get('ipresource')
                    if paramsstatus not in ["enable","disable"] or noderesource == None or ipresource == None:
                        return Response({'code':0,'msg':"Illegal input"},status=status.HTTP_400_BAD_REQUEST) 
                    if ipresource != None and len(ipresource) !=0:
                        tb_fact_device_info.objects.filter(vip_address=ipresource).update(vip_enable_switch=paramsstatus)
                        tb_fact_device_info.objects.filter(vip_address=ipresource).update(vip_status=paramsstatus)
                        return Response({'code':1,'msg':"update ok"},status=status.HTTP_200_OK)
                    if noderesource != None and len(noderesource) !=0:
                        tb_fact_device_info.objects.filter(node_id=noderesource).update(vip_enable_switch=paramsstatus)
                        tb_fact_device_info.objects.filter(node_id=noderesource).update(vip_status=paramsstatus)
                        return Response({'code':1,'msg':"update ok"},status=status.HTTP_200_OK)
                else:
                    return Response({'code':0,'msg':"Illegal input"},status=status.HTTP_400_BAD_REQUEST)
        except Exception as err:
    ...
    @action(detail=False,methods=['post'])
    def adjust_resources(self,request):
        record_list = []
        try:
            with transaction.atomic():            
                querydic = eval(request.body)
        ...
        try:
            with transaction.atomic():            
                ...
                if querydic != None and len(querydic) ==4:
                    ...
                    detectstatus = querydic.get('detectstatus')
                    artificialstatus = querydic.get('artificialstatus')
                    if detectstatus not in ["enable","disable"] or artificialstatus not in ["enable","disable"] or noderesource == None or ipresource == None:
                        return Response({'code':0,'msg':"Illegal input"},status=status.HTTP_400_BAD_REQUEST) 
                    if ipresource != None and len(ipresource) !=0:
                        tb_fact_device_info.objects.filter(vip_address=ipresource).update(vip_enable_switch=detectstatus) 
                        tb_fact_device_info.objects.filter(vip_address=ipresource).update(vip_status=artificialstatus) 
                        return Response({'code':1,'msg':"update ok"},status=status.HTTP_200_OK)
                    if noderesource != None and len(noderesource) !=0:
                        tb_fact_device_info.objects.filter(node_id=noderesource).update(vip_enable_switch=detectstatus)
                        tb_fact_device_info.objects.filter(node_id=noderesource).update(vip_status=artificialstatus)
                        return Response({'code':1,'msg':"update ok"},status=status.HTTP_200_OK)
                else:
                    return Response({'code':0,'msg':"Illegal input"},status=status.HTTP_400_BAD_REQUEST)
        except Exception as err:
...
class Viewinfo(MyModelViewSet):
    ...
    def get_location_info(self,request):
        try:
            fatherid = request.GET.get('viewid')
            if fatherid == None or len(fatherid) == 0:
                return Response({'code':0,'msg':"Illegal input"},status=status.HTTP_400_BAD_REQUEST)  
            queryset = tb_fact_temp_view_info.objects.filter(view_grade_name='default')
            defaultid = queryset[0].id
            fatherid_list = []
            while True: 
                queryset = tb_fact_temp_view_info.objects.filter(id=fatherid)
                if len(queryset) !=0:
                    fatherid = queryset[0].view_father_id
                    fatherid_list.append(fatherid)
                else:   
                    break   
            queryset = tb_fact_temp_view_info.objects.filter(id__in=fatherid_list)
            serializer = self.serializer_class(queryset,many=True)
            return Response({'code':1,'msg': serializer.data},status=status.HTTP_200_OK)
        except Exception as err:
            return Response({'code':0,'msg':str(err)},status=status.HTTP_400_BAD_REQUEST)
    ...
    def get_child_region(self,request):
     ...
     #通过关键字查找item
    @action(detail=False,methods=['get'])
    def get_child_region_temp(self,request):
        try:
            view_grade = request.GET.get('view_grade', None)
            fatherid = request.GET.get('fatherid')
            country = request.GET.get('country', None)
            isp = request.GET.get('isp', None)
            region = request.GET.get('region', None)
            ...
            if country != None:
                queryset = queryset.filter(view_country__contains = country)
            if isp != None:
                queryset = queryset.filter(view_isp=isp)
            if region != None:
                queryset = queryset.filter(view_region__contains=region)
            if province != None:
                 queryset = queryset.filter(view_region__contains=region)
            ...
            if city != None:
                queryset = queryset.filter(view_city__contains=city)
            if view_grade != None:
                queryset = queryset.filter(view_grade=view_grade)
            serializer = self.serializer_class(queryset,many=True)
            return Response({'code':1,'msg': serializer.data},status=status.HTTP_200_OK)
        except Exception as err:
            return Response({'code':0,'msg':str(err)},status=status.HTTP_400_BAD_REQUEST)
...
class Nameidinfo(MyModelViewSet):
    ...
    def push_nameid(self,request):
        try:
            nameid_id = request.GET.get('nameid')
            if nameid_id == None or len(nameid_id) == 0:
                return Response({'code':0,'msg':"Illegal input"},status=status.HTTP_400_BAD_REQUEST)
            with transaction.atomic():
                res = tb_dimension_nameid_view_info.objects.filter(Q(nameid_id__nameid_name=nameid_id) & Q(nameid_view_id__view_grade_name='default'))
                if len(res)==0:
                    raise RuntimeError('the default view or nameid is not exixts (nameid view)')
                queryset = tb_dimension_nameid_view_device_info.objects.filter(Q(nameid_id__nameid_name=nameid_id) & Q(nameid_view_id__view_grade_name='default'))
                cnamequeryset = tb_dimension_nameid_view_cname_info.objects.filter(Q(nameid_id__nameid_name=nameid_id) & Q(nameid_view_id__view_grade_name='default'))
                if len(cnamequeryset) == 0 and len(queryset) == 0:
                    raise RuntimeError('the default view or nameid is not exixts (nameid view cname device)')
                tb_fact_nameid_info.objects.filter(nameid_name=nameid_id).update(nameid_status="enable")
                return Response({'code':1,'msg':"ok"},status=status.HTTP_200_OK)
        except Exception as err:
            return Response({'code':0,'msg':str(err)},status=status.HTTP_400_BAD_REQUEST) 
    @action(detail=False,methods=['get'])
    ...
    def copy_name(self,request):
        ...
        try:
            nameid = request.GET.get('nameid')
            oldnameid = request.GET.get('oldnameid')
            if nameid == None or len(nameid) == 0 or oldnameid == None or len(oldnameid) == 0:
                return Response({'code':0,'msg':"Illegal input"},status=status.HTTP_400_BAD_REQUEST)
            with transaction.atomic():
                queryset = tb_fact_nameid_info.objects.filter(nameid_name = nameid)
                oldqueryset = tb_fact_nameid_info.objects.filter(nameid_name = oldnameid)
                if len(queryset) == 0 or len(oldqueryset) == 0:
                    raise RuntimeError('the nameid is not exixts')
                else:     
                    newnameidobj = queryset[0].id
                    viewres = tb_dimension_nameid_view_info.objects.filter(nameid_id_id=newnameidobj)
                    viewdevres = tb_dimension_nameid_view_device_info.objects.filter(nameid_id_id=newnameidobj)
                    viewcnameres = tb_dimension_nameid_view_cname_info.objects.filter(nameid_id_id=newnameidobj)
                    if len(viewres) != 0 or len(viewdevres) !=0 or len(viewcnameres) !=0:
                        return Response({'code':0,'msg':str("the new nameid has exists resource")},status=status.HTTP_400_BAD_REQUEST)
                    oldnameidobj = oldqueryset[0].id
                    querysetview = tb_dimension_nameid_view_info.objects.filter(nameid_id_id=oldnameidobj) 
                    if querysetview and len(querysetview) != 0:
                        for objview in querysetview:
                            ...
                            if not serializer.is_valid():
                                raise serializers.ValidationError
                            serializer.save()
                        querysetviewdev = tb_dimension_nameid_view_device_info.objects.filter(nameid_id_id=oldnameidobj)
                        if querysetviewdev and len(querysetviewdev) !=0:
                            for objviewdev in querysetviewdev:
                                ...
                                if not serializer.is_valid():
                                    ...
                                    serializer.save()
                                    record_list.append(serializer.data)
                        querysetviewcname = tb_dimension_nameid_view_cname_info.objects.filter(nameid_id_id=oldnameidobj)
                        ...
                        raise RuntimeError('the old nameid view is not exixts')
        except Exception as err:
            return Response({'code':0,'msg':str(err)},status=status.HTTP_400_BAD_REQUEST)
        return Response({'code':1,'msg':record_list},status=status.HTTP_200_OK)
...
class NameidViewinfo(MyModelViewSet):
    ...
    @action(detail=False,methods=['get'])
    def push_nameid_viewid(self,request):
        try:
            nameid_id = request.GET.get('nameid')
            view_id = request.GET.get('viewid')
            if nameid_id == None or len(nameid_id) == 0 or view_id == None or len(view_id) == 0 or nameid_id == -1 or view_id == -1:
                return Response({'code':0,'msg':"Illegal input"},status=status.HTTP_400_BAD_REQUEST)
            with transaction.atomic():
                queryset = tb_dimension_nameid_view_info.objects.filter(Q(nameid_id=nameid_id) & Q (nameid_view_id=view_id))
                dev_queryset = tb_dimension_nameid_view_device_info.objects.filter(Q(nameid_id=nameid_id) & Q (nameid_view_id=view_id))
                cname_queryset = tb_dimension_nameid_view_cname_info.objects.filter(Q(nameid_id=nameid_id) & Q (nameid_view_id=view_id))
                if len(queryset) == 0 and len(dev_queryset) == 0 and len(cname_queryset) == 0:
                    raise RuntimeError('the view and resource is null ')
                if len(dev_queryset) != 0 and len(cname_queryset) !=0:
                    raise RuntimeError('the ip and cname is not both ')
                if len(dev_queryset) == 0 and len(cname_queryset) ==0:
                    raise RuntimeError('the ip and cname is blank ')
                if dev_queryset and len(dev_queryset) != 0:
                    tb_dimension_backend_nameid_view_device_info.objects.filter(Q(nameid_id=nameid_id) & Q (nameid_view_id=view_id)).delete()
                    sql = 'insert into Polaris_tb_dimension_backend_nameid_view_device_info(nameid_id_id,nameid_view_id_id,nameid_device_id_id,nameid_device_ratio,nameid_device_status) select nameid_id_id,nameid_view_id_id,nameid_device_id_id,nameid_device_ratio,nameid_device_status from Polaris_tb_dimension_nameid_view_device_info where nameid_id_id={} and nameid_view_id_id={}'.format(nameid_id,view_id)
                    res = my_custom_sql(sql)
                if cname_queryset and len(cname_queryset) !=0:
                    tb_dimension_backend_nameid_view_cname_info.objects.filter(Q(nameid_id=nameid_id) & Q (nameid_view_id=view_id)).delete()
                    sql = 'insert into Polaris_tb_dimension_backend_nameid_view_cname_info(nameid_id_id,nameid_view_id_id,nameid_cname_id_id,nameid_cname_ratio,nameid_cname_status) select nameid_id_id,nameid_view_id_id,nameid_cname_id_id,nameid_cname_ratio,nameid_cname_status from Polaris_tb_dimension_nameid_view_cname_info where nameid_id_id={} and nameid_view_id_id={}'.format(nameid_id,view_id)
    ...
    @action(detail=False,methods=['get'])
    def get_accurate_view_byquery_vue(self,request):
        try:
            nameid = request.GET.get('nameid')
            ...
            page = self.paginate_queryset(queryset)
            if page is not None:
                serializer = NameidViewListSerializer(page, many=True)
                return self.get_paginated_response(serializer.data)
        ...
        try:
            with transaction.atomic():
                ...
                if nameid == None or len(nameid) == 0 or viewid == None or len(viewid) == 0:
                    return Response({'code':0,'msg':"Illegal input"},status=status.HTTP_400_BAD_REQUEST)
                res = tb_dimension_nameid_view_info.objects.filter(Q(nameid_id=nameid) & Q(nameid_view_id=viewid) & Q(nameid_view_id__view_grade_name='default'))
                if res and len(res) !=0:
                    raise RuntimeError('can not delete the default view ')
                ...
                tb_dimension_backend_nameid_view_device_info.objects.filter(Q(nameid_id=nameid) & Q(nameid_view_id=viewid)).delete()
                tb_dimension_nameid_view_cname_info.objects.filter(Q(nameid_id=nameid) & Q(nameid_view_id=viewid)).delete()
                tb_dimension_backend_nameid_view_cname_info.objects.filter(Q(nameid_id=nameid) & Q(nameid_view_id=viewid)).delete()
                return Response({'code':1,'msg':"del ok"},status=status.HTTP_200_OK)        
        except Exception as err:
            return Response({'code':0,'msg':str(err)},status=status.HTTP_400_BAD_REQUEST)        
        return Response({'code':1,'msg':""},status=status.HTTP_200_OK)
...
class NameidViewDeviceBackendinfo(MyModelViewSet):
    queryset = tb_dimension_backend_nameid_view_device_info.objects.all()
    serializer_class = NameidViewDeviceBackendSerializer 
class NameidViewCnameBackendinfo(MyModelViewSet):
    queryset = tb_dimension_backend_nameid_view_cname_info.objects.all()
    serializer_class = NameidViewCnameBackendSerializer 
...
class NameidViewDeviceinfo(MyModelViewSet):
    ...
    def delete_relation_resource(self,request):
        try:
            with transaction.atomic():
                ...
                if nameid == None or len(nameid) == 0 or viewid == None or len(viewid) == 0 or devid == None or len(devid)==0:
                    return Response({'code':0,'msg':"Illegal input"},status=status.HTTP_400_BAD_REQUEST)
                tb_dimension_nameid_view_device_info.objects.filter(Q(nameid_id=nameid) & Q(nameid_view_id=viewid) & Q(nameid_device_id=devid)).delete()
                tb_dimension_backend_nameid_view_device_info.objects.filter(Q(nameid_id=nameid) & Q(nameid_view_id=viewid) & Q(nameid_device_id=devid)).delete()
                return Response({'code':1,'msg':"del ok"},status=status.HTTP_200_OK)        
        except Exception as err:
            return Response({'code':0,'msg':str(err)},status=status.HTTP_400_BAD_REQUEST)        
        return Response({'code':1,'msg':""},status=status.HTTP_200_OK)
    ...
    def get_accurate_by_query(self,request):
        try: 
            ...
            if viewid != None:
                queryset = queryset.filter(nameid_view_id=viewid)
            if nodeid != None and len(nodeid) !=0:
            ...
            page = self.paginate_queryset(queryset)
            if page is not None:
                serializer = NameidViewDeviceListSerializer(page, many=True)
                return self.get_paginated_response(serializer.data)
    ...
    def get_info_by_nameid(self,request):
        try: 
            nameid = request.GET.get('nameid', None)
            queryset = tb_dimension_backend_nameid_view_device_info.objects.filter(Q(nameid_id = nameid)) 
            serializer = NameidViewDeviceListSerializer(queryset,many=True)
            return Response({'code':1,'msg':serializer.data},status=status.HTTP_200_OK)        
        except Exception as err:
            return Response({'code':0,'msg':str(err)},status=status.HTTP_400_BAD_REQUEST)        
...
class NameidViewCnameinfo(MyModelViewSet):
    ...
    def delete_relation_resource(self,request):
        try:
            with transaction.atomic():
                nameid = request.GET.get('nameid')
                viewid = request.GET.get('viewid')
                cnameid = request.GET.get('cnameid')
                if nameid == None or len(nameid) == 0 or viewid == None or len(viewid) == 0 or cnameid == None or len(cnameid)==0:
                    return Response({'code':0,'msg':"Illegal input"},status=status.HTTP_400_BAD_REQUEST)
                tb_dimension_nameid_view_cname_info.objects.filter(Q(nameid_id=nameid) & Q(nameid_view_id=viewid) & Q(nameid_cname_id=cnameid)).delete()
                tb_dimension_backend_nameid_view_cname_info.objects.filter(Q(nameid_id=nameid) & Q(nameid_view_id=viewid) & Q(nameid_cname_id=cnameid)).delete()
                return Response({'code':1,'msg':"del ok"},status=status.HTTP_200_OK)        
        except Exception as err:
            return Response({'code':0,'msg':str(err)},status=status.HTTP_400_BAD_REQUEST)        
        return Response({'code':1,'msg':""},status=status.HTTP_200_OK)
    ...
    def get_accurate_by_query(self,request):
        try:
            ...
            page = self.paginate_queryset(queryset)
            if page is not None:
                serializer = NameidViewCnameListSerializer(page, many=True)
                return self.get_paginated_response(serializer.data)
    ...
    def get_info_by_nameid(self,request):
        try: 
            nameid = request.GET.get('nameid', None)
            queryset = tb_dimension_backend_nameid_view_cname_info.objects.filter(Q(nameid_id = nameid)) 
            serializer = NameidViewCnameListSerializer(queryset,many=True)
            return Response({'code':1,'msg':serializer.data},status=status.HTTP_200_OK)        
        except Exception as err:
            return Response({'code':0,'msg':str(err)},status=status.HTTP_400_BAD_REQUEST)        
    ...
    def get_all_adminip(self,request):
         try:
            ...
            if nodeid != None:
                queryset = queryset.filter(node_id__contains=nodeid)
            if adminip != None:
                queryset = queryset.filter(admin_ip__contains=adminip)
            if country != None:
...
class DetectDeviceAvailabilityStandardInfo(MyModelViewSet):
    ...
    def universal_matching_standard(self,request):
        try:            
            ...
            return Response({'code':0,'msg':str(err)},status=status.HTTP_400_BAD_REQUEST)
tb_fact_user_info
class UserInfo(MyModelViewSet):
    queryset = tb_fact_user_info.objects.all()
...
import os
import json
def static_get_list(request):
    curPath = os.path.dirname(os.path.realpath(__file__))
  #  yamlPath = os.path.join(curPath, "index.yaml")
    url = request.GET.get('url','')
    print(url)
    yamlPath = "/home/wangli.wellwang/GLB/polaris/static_files/index.yaml"
    f = open(yamlPath, 'r', encoding='utf-8')
    cfg = f.read()
    d = yaml.load(cfg)  # 用load方法转字典
    
    return HttpResponse(json.dumps(d['paths'][url]),content_type="application/json")
  #  return HttpResponse(json.dumps(d['paths']['/sys/menu/nav']),content_type="application/json")
#前端获取指定域名的解析信息
