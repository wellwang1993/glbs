<template>
  <div class="app-container">
    <div class="filter-container" :model="search_temp">
      <el-input v-model="search_temp.policyname" placeholder="policyname" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    <el-dialog :title="textMap[dialogDeleteStatus]" :visible.sync="dialogDeleteFormVisible">
      ...
      >
        ...
        <el-table-column label="policy_name" width="350px" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.policy_name }}</span>
          </template>
        </el-table-column>
        <el-table-column label="policy_status" width="350px" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.policy_status }}</span>
          </template>
        </el-table-column>
        <el-table-column label="policy_describe" width="350px" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.policy_describe }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :model="temp" label-position="left" label-width="120px" style="width: 400px; margin-left:50px;">
        <el-form-item label="policy_name" prop="record">
          <el-input v-model="temp.policy_name" />
        </el-form-item>
        <el-form-item label="policy_status" prop="record">
          <el-select v-model="temp.policy_status" class="filter-item" placeholder="Please select">
            <el-option v-for="item in status_swicth" :key="item" :label="item" :value="item" />
          </el-select>
        </el-form-item>
        <el-form-item label="policy_describe" prop="record">
          <el-input v-model="temp.policy_describe" />
        </el-form-item>
      </el-form>
...
<script>
import { deleteid, fetchList, create, update } from '@/api/policy'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchList({ 'policyname': this.search_temp.policyname, 'page': this.page, 'size': this.limit }).then(response => {
