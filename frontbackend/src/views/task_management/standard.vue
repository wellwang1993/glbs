<template>
  <div class="app-container">
    <div class="filter-container" :model="search_temp">
      <el-input v-model="search_temp.node_isp" placeholder="nodeisp" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    <el-dialog :title="textMap[dialogDeleteStatus]" :visible.sync="dialogDeleteFormVisible">
      ...
      >
        ...
        <el-table-column label="total_value" width="350px" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.total_value }}</span>
          </template>
        </el-table-column>
        <el-table-column label="absolute_value" width="350px" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.absolute_value }}</span>
          </template>
        </el-table-column>
        <el-table-column label="relative_rate" width="350px" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.relative_rate }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :model="temp" label-position="left" label-width="120px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="total_value" prop="record">
          <el-input v-model="temp.total_value" />
        </el-form-item>
        <el-form-item label="absolute_value" prop="record">
          <el-input v-model="temp.absolute_value" />
        </el-form-item>
        <el-form-item label="relative_rate" prop="record">
          <el-input v-model="temp.relative_rate" />
        </el-form-item>
      </el-form>
...
<script>
import { deleteid, fetchList, create, update } from '@/api/standard'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchList({ 'standard': this.search_temp.node_isp, 'page': this.page, 'size': this.limit }).then(response => {
