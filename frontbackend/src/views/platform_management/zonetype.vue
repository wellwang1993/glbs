  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="zonename" width="350px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.zone_name }}</span>
        </template>
      </el-table-column>

      <el-table-column label="zonestatus" width="350px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.zone_status }}</span>
        </template>
      </el-table-column>
      <el-table-column label="zonescribe" width="350px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.zone_describe }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        <el-form-item label="Name" prop="zonename">
          <el-input v-model="temp.zone_name" />
        </el-form-item>
        <el-form-item label="Status" prop="zonestatus">
          <el-select v-model="temp.zone_status" class="filter-item" placeholder="Please select">
            <el-option v-for="item in status_swicth" :key="item" :label="item" :value="item" />
          </el-select>
        </el-form-item>
        <el-form-item label="Describe">
          <el-input v-model="temp.zone_describe" :autosize="{ minRows: 2, maxRows: 4}" type="textarea" placeholder="Please input" />
        </el-form-item>
      </el-form>
...
<script>
import { deleteid, fetchList, create, update } from '@/api/zonetype'
...
export default {
  ...
  data() {
    return {
      ...
      pvData: [],
      rules: {
        zonename: [{ required: true, message: 'zone name is required', trigger: 'blur' }],
        zonestatus: [{ required: true, message: 'status is required', trigger: 'blur' }]
      }
    }
  },
  created() {
  ...
  methods: {
    getList() {
      ...
      fetchList({ 'zonetype': this.search_temp.zone_name, 'page': this.page, 'size': this.limit }).then(response => {
