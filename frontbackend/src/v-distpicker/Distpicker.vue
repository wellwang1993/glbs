<template>
  <div :class="wrapper">
    <template v-if="type != 'mobile'">
      <select @change="getCountrys" v-model="currentDefault" :disabled="disabled || defaultDisabled">
        <option :value="placeholders.default">{{ placeholders.default }}</option>
        <option v-for="(item, index) in defaults"
                :value="item"
                :key="index">
          {{ item }}
        </option>
...
export default {
  ...
  props: {
    ...
    isp: { type: [String, Number], default: '' },
  ...
  data() {
    return {
      ...
      cities: [],
      areas: [],
      currentDefault: this.determineType(this.default) || this.placeholders.default,
  ...
  created() {
    if (this.type != 'mobile') {
      ...
      this.defaults = this.getDistricts()
      // debugger 
      console.log('aaaaaa')
      console.log(this.default)
      console.log('abbbaaaaa')
      console.log(this.province)
      console.log('ccccabbbaaaaa')
      this.countrys = this.default ? this.getDistricts(this.getAreaCode(this.determineType(this.default))) : []
      // debugger 
      console.log(this.defaults)
      console.log(this.countrys)
      // this.isp = this.country ? this.getDistricts(this.getAreaCode(this.determineType(this.country))) : []
      // this.province = this.isp ? this.getDistricts(this.getAreaCode(this.determineType(this.isp))) : []
  ...
  watch: {
    currentCountry(vaule) {
      this.$emit('country', this.setData(vaule))
    ...
    country(value) {
      this.currentCountry = this.country || this.placeholders.country
  ...
  methods: {
    ...
    handleFilter() {
       ...
       var tag = 0
       if (this.currentDefault != '默认' ) {
         tag = 1
         fatherid = this.getAreaCodeByPreCode(this.currentDefault,fatherid)
         console.log(fatherid)
       }
       ...
       this.$emit('getAddViewId',fatherid);
       // this.$emit('getAddViewId',[fatherid,fatherlist]);
    },
    setData(value, check = '') {
      return {
    ...
    getCountrys() {
       ...
       this.currentCity = this.placeholders.city
       this.currentArea = this.placeholders.area
       this.countrys = this.determineValue(this.currentDefault, this.placeholders.default)
       this.cleanList('isps')
